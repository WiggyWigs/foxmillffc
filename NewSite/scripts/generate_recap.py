...
def build_gotw_writing_prompt(matchup, tone):
    ...
    system = (
        ...
        "When referring to a manager's standings position, use "
        "natural English word order for whichever sentence shape "
        "you're writing: if the ordinal directly modifies a name or "
        "noun that follows it (attributive), put the ordinal first, "
        "like a compound adjective — e.g. '2nd ranked Brian "
        "Kleinhenz' or 'the 5th ranked squad'. If it follows a "
        "linking verb like 'sits' or 'is' (predicate), put 'ranked' "
        "first and the ordinal after — e.g. 'sits ranked 12th' or "
        "'is ranked 8th', never 'sits 12th ranked'. Never use a bare "
        "number without the word 'ranked' at all, e.g. never 'rank "
        "2' or 'at rank 5'. IMPORTANT: season_schedule_difficulty "
